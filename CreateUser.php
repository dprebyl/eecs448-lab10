...
<html class="h-100">
	<head>
		<meta charset="utf-8">
		<title>Create User - EECS 448 Lab 10</title>
	...
	<body class="h-100 d-flex flex-column">
		<nav class="navbar navbar-expand navbar-dark bg-dark border-bottom" style="flex-shrink: none">
			...
			<ul class="navbar-nav">
				<li class="nav-item">
					<a class="nav-link active" href="CreateUser.html">2: CreateUser</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" href="CreatePosts.html">3: CreatePosts</a>
				</li>
				<li class="nav-item">
		...
		<div class="container" style="flex: 1 0 auto">
			...
			<?php
				...
				if ($user_id == "") error("User ID cannot be blank");
				else {
					...
					if ($mysqli->connect_errno) error("Database connection failed: " . $mysqli->connect_error);
						...
						$query->fetch();
						$query->close();
						if ($count > 0) error("User ID already taken");
						else {
							...
							$query->close();
							?>
								<span class="text-success">User "<?=$user_id?>" created</span>
							<?php
						}
					}
					$mysqli->close();
